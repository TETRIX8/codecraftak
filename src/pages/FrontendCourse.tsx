...
export default function FrontendCourse() {
  ...
  const selectedLesson = useMemo(
    () => frontendCourse.lessons.find(item => item.id === lessonId),
    [lessonId],
  );
  const progress = Math.round((completed.length / frontendCourse.lessons.length) * 100);

  const toggleCompleted = (id: string) => {
    setCompleted(current => current.includes(id) ? current.filter(item => item !== id) : [...current, id]);
  };

  const openLesson = (id: string) => navigate(`/courses/${frontendCourse.slug}/${id}`);
        ...
        <main className="container mx-auto grid max-w-5xl gap-6 px-4 py-8 lg:grid-cols-[minmax(0,1fr)_280px]">
          <article className="min-w-0 rounded-2xl border border-border/50 bg-card/60 p-5 shadow-xl shadow-primary/5 sm:p-8">
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              {nextLesson ? <Button variant="outline" onClick={() => openLesson(nextLesson.id)}>Следующее занятие <ChevronRight className="ml-2 h-4 w-4" /></Button> : <Button variant="outline" onClick={() => navigate(`/courses/${frontendCourse.slug}`)}>Завершить курс</Button>}
            </div>
          </article>
          <aside className="lg:sticky lg:top-6 lg:self-start">
            <Card className="border-border/50 bg-card/70">
              <CardHeader><CardTitle className="text-base">Прогресс курса</CardTitle></CardHeader>
              <CardContent><div className="flex items-end justify-between"><span className="text-3xl font-bold">{progress}%</span><span className="text-sm text-muted-foreground">{completed.length}/{frontendCourse.lessons.length}</span></div><Progress value={progress} className="mt-3" /><div className="mt-5 space-y-2">{frontendCourse.lessons.map(item => <button key={item.id} onClick={() => openLesson(item.id)} className={`flex w-full items-center gap-2 rounded-lg p-2 text-left text-xs transition-colors hover:bg-muted ${item.id === selectedLesson.id ? 'bg-primary/10 text-primary' : ''}`}><span className="w-5 text-muted-foreground">{item.number}</span><span className="min-w-0 flex-1 truncate">{item.title}</span>{completed.includes(item.id) && <Check className="h-3.5 w-3.5 text-green-400" />}</button>)}</div></CardContent>
            </Card>
          </aside>
        </main>
      </div>
