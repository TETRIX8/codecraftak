  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="container mx-auto px-3 sm:px-4 pb-8 sm:pb-16">
        ...
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="mb-6 overflow-hidden rounded-2xl border border-slate-500/40 bg-gradient-to-br from-slate-500/20 via-card/80 to-cyan-500/10 shadow-xl shadow-cyan-500/5">
          <div className="grid gap-6 p-5 sm:p-7 lg:grid-cols-[1fr_auto] lg:items-center">
            <div><div className="mb-3 flex flex-wrap items-center gap-2"><Badge className="gap-1 bg-slate-500/20 text-slate-200" variant="outline"><Terminal className="h-3 w-3" /> Системная дисциплина</Badge><Badge variant="outline">{systemSoftwareCourse.totalHours} часов</Badge><Badge variant="outline">{systemSoftwareCourse.lessons.length} уроков</Badge></div><h2 className="text-2xl font-bold sm:text-3xl">{systemSoftwareCourse.title}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">{systemSoftwareCourse.description} Память, указатели, C++23, процессы, файловая система, потоки и синхронизация.</p><div className="mt-4 flex flex-wrap gap-2">{systemSoftwareStages.map(stage => <span key={stage.name} className="rounded-full border border-border/60 bg-background/40 px-3 py-1 text-xs text-muted-foreground">{stage.name} · {stage.hours} ч.</span>)}</div></div>
            <Button size="lg" className="gap-2" variant="outline" onClick={() => navigate(`/courses/${systemSoftwareCourse.slug}`)}><Terminal className="h-4 w-4" /> Открыть Системное ПО</Button>
          </div>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="mb-6 overflow-hidden rounded-2xl border border-orange-500/30 bg-gradient-to-br from-orange-500/15 via-card/80 to-red-500/10 shadow-xl shadow-orange-500/5"><div className="grid gap-6 p-5 sm:p-7 lg:grid-cols-[1fr_auto] lg:items-center"><div><div className="mb-3 flex flex-wrap items-center gap-2"><Badge className="gap-1 bg-orange-500/15 text-orange-300" variant="outline"><Terminal className="h-3 w-3" /> Сетевая безопасность</Badge><Badge variant="outline">{networkSecurityCourse.totalHours} часов</Badge><Badge variant="outline">{networkSecurityCourse.lessons.length} уроков</Badge></div><h2 className="text-2xl font-bold sm:text-3xl">{networkSecurityCourse.title}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">{networkSecurityCourse.description}</p><div className="mt-4 flex flex-wrap gap-2">{networkSecurityStages.map(stage => <span key={stage.name} className="rounded-full border border-border/60 bg-background/40 px-3 py-1 text-xs text-muted-foreground">{stage.name} · {stage.hours} ч.</span>)}</div></div><Button size="lg" variant="outline" className="gap-2 border-orange-500/40 hover:bg-orange-500/10" onClick={() => navigate(`/courses/${networkSecurityCourse.slug}`)}><Terminal className="h-4 w-4" /> Открыть курс</Button></div></motion.div>

        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="mb-6 overflow-hidden rounded-2xl border border-violet-500/30 bg-gradient-to-br from-violet-500/15 via-card/80 to-fuchsia-500/10 shadow-xl shadow-violet-500/5"><div className="grid gap-6 p-5 sm:p-7 lg:grid-cols-[1fr_auto] lg:items-center"><div><div className="mb-3 flex flex-wrap items-center gap-2"><Badge className="gap-1 bg-violet-500/15 text-violet-300" variant="outline"><Code className="h-3 w-3" /> Enterprise-разработка</Badge><Badge variant="outline">{enterpriseDevelopmentCourse.totalHours} часов</Badge><Badge variant="outline">{enterpriseDevelopmentCourse.lessons.length} уроков</Badge></div><h2 className="text-2xl font-bold sm:text-3xl">{enterpriseDevelopmentCourse.title}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">{enterpriseDevelopmentCourse.description}</p><div className="mt-4 flex flex-wrap gap-2">{enterpriseDevelopmentStages.map(stage => <span key={stage.name} className="rounded-full border border-border/60 bg-background/40 px-3 py-1 text-xs text-muted-foreground">{stage.name} · {stage.hours} ч.</span>)}</div></div><Button size="lg" variant="outline" className="gap-2 border-violet-500/40 hover:bg-violet-500/10" onClick={() => navigate(`/courses/${enterpriseDevelopmentCourse.slug}`)}><Code className="h-4 w-4" /> Открыть курс</Button></div></motion.div>

        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="mb-6 overflow-hidden rounded-2xl border border-emerald-500/30 bg-gradient-to-br from-emerald-500/15 via-card/80 to-teal-500/10 shadow-xl shadow-emerald-500/5"><div className="grid gap-6 p-5 sm:p-7 lg:grid-cols-[1fr_auto] lg:items-center"><div><div className="mb-3 flex flex-wrap items-center gap-2"><Badge className="gap-1 bg-emerald-500/15 text-emerald-300" variant="outline"><Sparkles className="h-3 w-3" /> Интеллектуальные системы</Badge><Badge variant="outline">{intelligentSystemsCourse.totalHours} часов</Badge><Badge variant="outline">{intelligentSystemsCourse.lessons.length} уроков</Badge></div><h2 className="text-2xl font-bold sm:text-3xl">{intelligentSystemsCourse.title}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">{intelligentSystemsCourse.description}</p><div className="mt-4 flex flex-wrap gap-2">{intelligentSystemsStages.map(stage => <span key={stage.name} className="rounded-full border border-border/60 bg-background/40 px-3 py-1 text-xs text-muted-foreground">{stage.name} · {stage.hours} ч.</span>)}</div></div><Button size="lg" variant="outline" className="gap-2 border-emerald-500/40 hover:bg-emerald-500/10" onClick={() => navigate(`/courses/${intelligentSystemsCourse.slug}`)}><Sparkles className="h-4 w-4" /> Открыть курс</Button></div></motion.div>
