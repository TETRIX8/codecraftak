...
BEGIN
  ...
  FOR _badge IN SELECT * FROM badges LOOP
    ...
    IF _should_award AND NOT _already_has THEN
      ...
      PERFORM log_point_transaction(_user_id, _badge.reward_points, 'badge_reward', _badge.id, 'Награда за достижение: ' || _badge.name);
      badge_name := _badge.name;
      badge_icon := _badge.icon;
      newly_awarded := true;
      points_awarded := _badge.reward_points;
      RETURN NEXT;
